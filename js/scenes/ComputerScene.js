class ComputerScene extends Scene {
  ...
  start(untouchables) {
    ...
    const gaveup = document.querySelector('[data-action="gaveup"]');
    const again = document.querySelector('[data-action="again"]');
    gaveup.classList.remove("hidden");
    again.classList.add("hidden");
    this.removeEventListeners.push(
      addEventListener(gaveup, 'click', () => {
        this.app.start("preparation");
      })
    );
    this.removeEventListeners.push(
      addEventListener(again, 'click', () => {
        this.app.start("preparation");
      })
    );
  }
  ...
  update() {
    ...
    if(isEnd) {
      if (opponent.loser) {
        this.status.textContent = 'Ви перемогли!'
      } else {
    ...
    if (this.playerTurn) {
      ...
      this.status.textContent = 'Хід противника:';
    }
  }
}
